...
import { addReview, deleteReview, getReview,getUserReviews, getMovieReviews } 
...
describe('addReview', () => {
    ...
    it('should log an error if the review cannot be added', async () => {
        ...
        jest.spyOn(Movie, 'findOne').mockImplementation(() => { throw new Error('findOne error'); });
        const result = await addReview('1', '1', 'review', "5");
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith("Error adding review", new Error('findOne error'));
...
describe('deleteReview', () => {
    ...
    it('should log an error if the review cannot be deleted', async () => {
        ...
        jest.spyOn(Review, 'deleteOne').mockImplementation(() => { throw new Error('deleteOne error'); });
        const result = await deleteReview('1');
        expect(result).toBe(false);
        expect(consoleSpy).toHaveBeenCalledWith("Error deleting review", new Error('deleteOne error'));
        consoleSpy.mockRestore();
    });
});
...
describe('getReview', () => {
    ...
    it('should log an error if the review cannot be found', async () => {
        ...
        const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
        // Mock the findOne method to throw an error
        jest.spyOn(Review, 'findOne').mockImplementation(() => { throw new Error('findOne error'); });
        const result = await getReview('1');
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith("Error getting review", new Error('findOne error'));
        consoleSpy.mockRestore();
    });
});
...
describe('getMovieReviews', () => {
    ...
    it('should log an error if the reviews cannot be found', async () => {
        ...
        jest.spyOn(Review, 'find').mockImplementation(() => { throw new Error('find error'); });
        const result = await getMovieReviews('1');
        expect(result).toBeNull();
        expect(consoleSpy).toHaveBeenCalledWith("Error getting reviews", new Error('find error'));
        consoleSpy.mockRestore();
    });
});
